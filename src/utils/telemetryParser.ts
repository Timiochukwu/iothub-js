...
export const telemetryCodeMap: Record<string, string> = {
  ...
  export class TelemetryParser {
    ...
    private static parseNumericValue(data: Record<string, any>, keys: string[]): number | undefined {
    ...
    static validateTelemetryData(data: ParsedTelemetryData): string[] {
      ...
      if (data.position) {
        if (!ValidationUtils.isValidCoordinates(data.position.latitude, data.position.longitude)) {
      ...
      if (data.vehicle) {
        if (data.vehicle.speed && (data.vehicle.speed < 0 || data.vehicle.speed > 300)) {
          errors.push('Speed value out of valid range (0-300 km/h)');
        }
        
        if (data.vehicle.engineRpm && (data.vehicle.engineRpm < 0 || data.vehicle.engineRpm > 10000)) {
          errors.push('Engine RPM out of valid range (0-10000)');
        }
        
        if (data.vehicle.fuelLevel && (data.vehicle.fuelLevel < 0 || data.vehicle.fuelLevel > 100)) {
