    ...
    export class FuelAnalyticsService {
      ...
      async getCurrentFuelSummary(imei: string): Promise<{
        ...
        // Check for latest telemetry with fuel level AND VIN
        ...
        if (!latest?.state?.reported) {
          console.log(`No valid telemetry found for IMEI: ${imei} with fuel level and VIN`);
          return null;
        }
    
        // Extend lookback period for better refuel detection
        ...
        console.log(`Found ${recentData.length} fuel data points with VIN in last ${lookbackDays} days`);
    
        // Debug arrays
        const fuelChanges: Array<{
          from: number;
          to: number;
          change: number;
          timestamp: string;
        ...
        // Enhanced refuel detection with multiple thresholds
        let lastRefuel: { percentage: number; timestamp: number; date: string; time: string; } | undefined;
        
        if (recentData.length > 1) {
          // Try multiple thresholds from most to least strict
          const thresholds = [15, 10, 8, 5, 3]; // Start with 15%, go down to 3%
          
          for (const threshold of thresholds) {
            // Process all fuel changes for debug (only for current threshold)
            if (threshold === thresholds[0]) {
              for (let i = 1; i < recentData.length; i++) {
                const current = recentData[i]?.state?.reported;
                const previous = recentData[i - 1]?.state?.reported;
                
                if (current && previous) {
                  const currentLevel = current[AVL_ID_MAP.FUEL_LEVEL] || 0;
                  const previousLevel = previous[AVL_ID_MAP.FUEL_LEVEL] || 0;
            ...
            // Find refuel with current threshold (working backwards)
            for (let i = recentData.length - 1; i > 0; i--) {
              ...
              if (current && previous) {
                ...
                // Track candidates for debug
                if (increase >= 3) { // Log any increase >= 3%
                  refuelCandidates.push({
                ...
                if (increase >= threshold) {
                  const refuelTimestamp = new Date(current.ts);
                  lastRefuel = {
                    percentage: Math.round(increase * 10) / 10,
                    timestamp: refuelTimestamp.getTime(),
                    date: refuelTimestamp.toLocaleDateString('en-GB', { 
                      day: '2-digit', 
                      month: 'short', 
                      year: 'numeric' 
                    }),
                    time: refuelTimestamp.toLocaleTimeString('en-US', { 
                      hour: 'numeric', 
                      minute: '2-digit',
                      hour12: true 
                    })
                  };
                  console.log(`Found refuel with threshold ${threshold}%: +${increase}% at ${refuelTimestamp.toISOString()}`);
                  break;
                }
              }
            }
            
            // If we found a refuel, break out of threshold loop
            if (lastRefuel) break;
          }
        }
        ...
        const debugInfo = {
          ...
          fuelChanges: fuelChanges.slice(-20), // Show last 20 changes
          refuelCandidates: refuelCandidates.slice(-10), // Show last 10 candidates
          dataRange: {
            from: recentData.length > 0 ? new Date(recentData[0]?.state?.reported?.ts || 0).toISOString() : 'No data',
            to: recentData.length > 0 ? new Date(recentData[recentData.length - 1]?.state?.reported?.ts || 0).toISOString() : 'No data',
            totalPoints: recentData.length
          }
        };
    
        // Build result object conditionally to match TypeScript interface
        const result: any = {
        ...
        // Only add lastRefuel if it exists
        if (lastRefuel) {
          result.lastRefuel = lastRefuel;
        }
    
        // Debug logging before return
        console.log('Final result - lastRefuel included:', !!result.lastRefuel);
        console.log('lastRefuel object:', JSON.stringify(lastRefuel, null, 2));
