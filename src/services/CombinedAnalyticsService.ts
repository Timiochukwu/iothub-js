...
export class CombinedAnalyticsService {
    ...
    async getCombinedAnalyticsReport(
      ...
      const [drivingReportMap, fuelReportMap, batteryReportMap, fuelBarChartMap, tirePressureMap, engineHealthMap] =
        await Promise.all([
          this.drivingBehaviorService.getDrivingBehaviorReport(imei, startDate, endDate, type, options),
          this.fuelAnalyticsService.getFuelAnalyticsReport(imei, startDate, endDate, type),
          this.batteryAnalyticsService.getBatteryAnalyticsReport(imei, startDate, endDate, type),
          this.fuelAnalyticsService.getDailyFuelBarChartData(imei, startDate, endDate, type),
          this.tirePressureService.getDailyTirePressureData(imei, startDate, endDate, type),
          this.engineHealthService.getEngineHealthData(imei, startDate, endDate, type),
        ]);
    ...
    async getVehicleOverview(imei: string): Promise<{
      ...
      const [drivingStatus, batteryStatus, engineStatus, fuelStatus, tireStatus] = await Promise.all([
        this.drivingBehaviorService.getCurrentDrivingStatus ? this.drivingBehaviorService.getCurrentDrivingStatus(imei) : null,
        this.batteryAnalyticsService.getCurrentBatteryStatus(imei),
        this.engineHealthService.getEngineHealthData(imei, new Date(), new Date(), 'daily'),
        this.fuelAnalyticsService.getFuelAnalyticsReport(imei, new Date(), new Date(), 'daily'),
