...
export class DeviceService {
  async registerDevice(
    ...
    try {
      ...
      const existingDevice = await Device.findOne({
        $or: [{ imei: imei }, { $and: [{ imei: imei }, { vin: vin }] }],
      });
  ...
  async getUserDevices(
    ...
    page: number = 1,
    limit: number = 10
  ): Promise<ApiResponse<PaginatedResponse<IDevice>>> {
    try {
  ...
  async updateDevice(
    ...
    try {
      ...
      if (updateData.modelYear !== undefined)
        device.modelYear = updateData.modelYear;
      if (updateData.plateNumber !== undefined)
        device.plateNumber = updateData.plateNumber;
